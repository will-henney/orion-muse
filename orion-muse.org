...
#+END_SRC
**** TODO [4/5] Process spectral windows for each filter
...
#+BEGIN_SRC python
  intab = Table.read('all-filters-input.tab', format='ascii.tab')
#+END_SRC
***** TODO Extract the windows for each filter
#+BEGIN_SRC python
  <<bandpass-fullname-function>>
  for row in intab:
...
python clean_up_wav_wcs.py muse-hr-window-wfc3-f656n.fits
#+END_SRC

That seemed to work

#+BEGIN_SRC sh :results silent
python clean_up_wav_wcs.py muse-hr-window-acs-f658n.fits
#+END_SRC
